...
import gg.mooncraft.minecraft.prisoncells.FurnaceResumeTest;
...
public class MockFurnaceUtilities {

    /*
    Constants
     */
    private static String FUEL_KEY_FORMAT = "mc_fuel_%s";
    private static String FURNACE_KEY_FORMAT = "mc_furnace_%s";
    private static Map<NamespacedKey, Fuel> VANILLA_FUELS;
    private static Map<NamespacedKey, VirtualFurnaceRecipe> VANILLA_FURNACE_RECIPES;
    ...
    public static void registerAll() {
        ...
        FurnaceResumeTest.server.recipeIterator().forEachRemaining(recipe -> {
            if (recipe instanceof FurnaceRecipe furnaceRecipe) {
                VirtualFurnaceRecipe newFurnaceRecipe = new VirtualFurnaceRecipe(createKey(String.format(FURNACE_KEY_FORMAT, furnaceRecipe.getKey().getKey())), furnaceRecipe.getInput().getType(), furnaceRecipe.getResult().getType(), furnaceRecipe.getCookingTime(), furnaceRecipe.getExperience());
    ...
    Static Methods
     */
    private static void register(Map<NamespacedKey, Fuel> map, String name, Material fuel, int burnTicks) {
        Fuel newFuel = new Fuel(createKey(String.format(FUEL_KEY_FORMAT, name)), fuel, burnTicks);
        map.put(newFuel.getKey(), newFuel);
    }

    private static void register(Map<NamespacedKey, Fuel> map, String name, Tag<Material> fuelTag, int burnTicks) {
        Fuel newFuel = new Fuel(createKey(String.format(FUEL_KEY_FORMAT, name)), fuelTag, burnTicks);
        map.put(newFuel.getKey(), newFuel);
    }
    ...
    public static Fuel getFuelByMaterial(Material material) {
    ...
    public static VirtualFurnaceRecipe getRecipeByKey(NamespacedKey key) {
        return getVanillaFurnaceRecipes().get(key);
    }

    public static VirtualFurnaceRecipe getRecipeByIngredient(Material ingredient) {
    ...
    public static Map<NamespacedKey, Fuel> getVanillaFuels() {
        return VANILLA_FUELS;
    }

    public static Map<NamespacedKey, VirtualFurnaceRecipe> getVanillaFurnaceRecipes() {
        return VANILLA_FURNACE_RECIPES;
    }

    public static NamespacedKey createKey(String key) {
        return new NamespacedKey(MockBukkit.createMockPlugin(), key);
    }
}